...
Under the path ``~/environment/lucky-money-ar-workshop/amplify/backend/function/LuckyMoneyFunction/src``, add a new file called `lambda_function.py`. Add the following code in **lambda_function.py**.
...
**Save** the lambda_function.py file. Next, open the **~/environment/lucky-money-ar-workshop/amplify/backend/function/LuckyMoneyFunction/LuckyMoneyFunction-cloudformation-template.json** file and make these very specific changes:
...
When prompted to complete the changes. Enter Y and the deployment will begin.

You should receive a completion message when it is done deploying.
